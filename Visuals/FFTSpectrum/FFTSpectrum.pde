...
Star[] stars = new Star[1];  // the array containing the objects of the type "Star"
...
void setup() {
  ...
  // add the first star to stars the array
  stars[0] = new Star(random(-2, 2), random(-2, 2), random(-PI/2, PI/2));
}      
...
void draw() {
  ...
  // each 200 milliseconds add a new star in the stars array
